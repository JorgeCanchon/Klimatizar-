...
<div class="head1" class="derecha">
 ...
 <?php 
$jorge=conectar();
$produc=$_REQUEST['codigoProducto'];
$registro=$jorge->query("select nombreProveedor from  proveedor inner join proveedor_producto on NIT=Proveedor_NIT where Producto_codigoProducto='" . $produc . "'")or die($jorge->error);
...
$regis=$jorge->query("select nombreProducto from  producto where codigoProducto='" . $produc . "'")or die($jorge->error);
  $row=$registros->fetch_array();
    ...
    if ($row['nombreProveedor']=="") 
    {
      echo "<center>";
     echo "<h1>";
    echo 'Producto sin proveedor.' ;
    echo '<br>';
    echo '¿Desea agregar <a href="agregarProveedor.php?codigoProducto='.$produc.'">PROVEEDOR</a>?';
    echo "</h1>";
    echo "</center>";
